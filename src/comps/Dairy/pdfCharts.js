  ...
  return new Promise((resolve, rej) => {
    ...
    try {
      ...
      nutrientPromises = nutrientData.map((row, i) => {
        return createBarChart(row.key, nutrientLabels, row.data, area)
      })

      // Two Single Summary Charts
      totalNutrientAppAntiHarvestDataPromise = createBarChart(
      ...
      materialPromises = materialData.map((row, i) => {
        ...
        return createHoriBarChart(key, materialLabels, row.slice(0, -1), row.slice(-1), area)
      })
...
const createHoriBarChart = (key, labels, data, title, area) => {
...
const createBarChart = (key, labels, data, area) => {
  let canvas = document.createElement('canvas')
